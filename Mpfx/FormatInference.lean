...
This file proves the containment theorems at the *predicate* level:
for each operation we show that the result satisfies the three
`AbstractFormat` membership predicates (`precisionAtMost`, `quantumAtLeast`,
`boundOK`) at the inferred parameters.  Constructing the inferred format
as an `AbstractFormat` structure requires discharging the `p_pos`,
`not_degenerate`, and `b_nn` invariants, which depend on the specific
operands; we leave that to downstream users.

The four results, in order of complexity:

* `neg_mem` — `x ∈ F → -x ∈ F`.  (Already in `Mpfx.Format`; re-stated for completeness.)
* `abs_mem` — `x ∈ F → Dyadic.abs x ∈ F`.
* `mul_inferred` — `x ∈ F₁ → y ∈ F₂ → x · y` satisfies the inferred `⊗`-predicates.
* `add_inferred` — `x ∈ F₁ → y ∈ F₂ → x + y` satisfies the inferred `⊕`-predicates
  (with quantum and bound matching the paper exactly; precision left at `⊤`).
-/
