...
`hgt` says `F₂` uses strictly more digits at `z` than `F₁` does. This is the
natural digit-count form of Lemma 5.3's hypothesis: `F₂` resolves at finer
precision than `F₁` at the rounded value. -/
