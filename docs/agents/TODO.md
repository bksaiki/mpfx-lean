...
- All auxiliary hypotheses on `rndRTO_RNE` have been dropped: signature matches the paper exactly.
...
- **Drop `h_close` and `h_mid_in_F₂` auxiliary hypotheses on `rndRTO_RNE`.** **Status: both dropped, plus `F₁.exp` finite hypothesis dropped — signature is fully paper-aligned.**
  ...
  - **Done — Midpoint lemma** (`Mpfx/Format.lean`):
    ...
    - `AbstractFormat.midpoint_mem_extend_one_of_F_adjacent`: general (signed) midpoint-in-`F.extend 1` lemma. Case-splits on sign of `y₁`: positive uses `_pos` variant; `y₁ = 0` uses `half_mem_extend_one`; both negative uses negation symmetry; `y₁ < 0 < y₂` ruled out by F-adjacency. Covers all `(F.p, F.exp)` shapes.
  ...
  - **Done — `F.exp = ⊥` corner case** of midpoint lemma (`midpoint_mem_extend_one_of_F_adjacent_pos_exp_bot` and `midpoint_mem_extend_one_of_F_adjacent_exp_bot`, plus `exists_grid_rep_exp_bot`, `no_F_element_in_step_interval_exp_bot`, `F_adjacent_step_form_exp_bot` in Mpfx/Format.lean). Mirrors the F.exp finite proof but without the `max(exp, ...)` step.
  - **Done — `h_mid_in_F₂` derivation** (`midpoint_F₁_in_F₂_of_F_adjacent` in Mpfx/DoubleRounding.lean). Dispatches on `F₁.p` and `F₁.exp` shapes, applies the appropriate midpoint lemma + chain to `F₂`.
  - **Done — `rndRTO_RNE` refactored**: drops `h_mid_in_F₂`, `h_close`, and `F₁.exp` finite from the signature.
  - **Done — supporting helpers**: `extend_one_extend_one_subset_extend_two` (key lemma for the `notMem_of_extend_subset` application), `midpoint_in_F₁_extend_one_of_F_adjacent` (factored midpoint membership, dispatcher), `F_adjacent_of_RNE_round_pair` (F-adjacency of (w', z') from rounding clauses).
  - **Done — `h_close` derivation**: applies `notMem_of_extend_subset` at the `F₁.extend 1` level (using `extend_one_extend_one_subset_extend_two`) to derive `z ∉ F₁.extend 1`, hence `z ≠ midpoint(w', z')` (since the midpoint is in `F₁.extend 1`). Then the same-side-of-midpoint argument: RNE-closeness gives `z` on `w'`'s side of `m`, RTO + `m ∈ F₂` + `z ≠ m` forces `x` on the same side as `z` strictly. Hence `|x - w'| ≤ |x - z'|`.
