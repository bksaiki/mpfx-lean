...
- [x] **`rndRTO_RAZ` (general, all `x ∈ ℝ`), paper-aligned**: take `(hsub : F₁.extend 1 ⊆ F₂)` — paper's RAZ uses just `b₁` (no `next` bump), so the existing form already matches. Plus auxiliary `hp_F₂ : 2 ≤ F₂.p`.
